...
sub nickserv_notice {
    ...
    if (is_nickserv($server->{tag}, $address)) {
        if ($text =~ /^If this is your nickname, type \/msg NickServ/ || $text =~ /^This nickname is registered and protected.  If it is your/ || $text =~ /This nickname is registered\. Please choose a different nickname,/ || $text =~ /^This nickname is registered. Please choose a different nickname/ || $text =~ /^Ce pseudo est enregistr/) {
            ...
            Irssi::signal_stop();
        } elsif ($text =~ /^You have already identified/ || $text =~ /^This nick is already identified./ || $text =~ /^Vous êtes déjà identifié./) {
            Irssi::signal_stop();
        } elsif ($text =~ /^Password accepted - you are now recognized/ || $text =~ /^Mot de passe accepté/) {
            Irssi::signal_stop();
            Irssi::printformat(MSGLEVEL_CLIENTCRAP, 'password_accepted', $server->{tag});
        } elsif ($text =~ /^Password Incorrect/ || $text =~ /^Password incorrect./ || $text =~ /^Mot de passe incorrect./) {
            Irssi::signal_stop();
