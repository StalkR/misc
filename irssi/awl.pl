...
#     /set awl_visible <always|active|inactive>
#     /set awl_type <window|root>
...
sub add_statusbar {
	for (@_) {
		...
		# set visible
		if (lc Irssi::settings_get_str(set 'visible') eq 'always') {
			Irssi::command('statusbar ' . (set$_) . ' visible always');
		}
		elsif (lc Irssi::settings_get_str(set 'visible') eq 'active') {
			Irssi::command('statusbar ' . (set$_) . ' visible active');
		}
		elsif (lc Irssi::settings_get_str(set 'visible') eq 'inactive') {
			Irssi::command('statusbar ' . (set$_) . ' visible inactive');
		}
		# set type
		if (lc Irssi::settings_get_str(set 'type') eq 'window') {
			Irssi::command('statusbar ' . (set$_) . ' type window');
		}
		elsif (lc Irssi::settings_get_str(set 'type') eq 'root') {
			Irssi::command('statusbar ' . (set$_) . ' type root');
		}
		# set position
...
sub awlHasChanged () {
	...
	my $temp = ($SCREEN_MODE ?
		...
		Irssi::settings_get_str(set 'visible').
		Irssi::settings_get_str(set 'type').
...
Irssi::settings_add_str(setc, set 'visible', 'always');
Irssi::settings_add_str(setc, set 'type', 'root');
...
#
# 0.6cb
# - statusbar visible/type settings
# 
# 0.6ca
